...
def main():
    ...
    try:
        if os.path.isdir(sys.argv[1]):
            dirpath = sys.argv[1]
            runpaths = [dirpath + "/" + path for path in os.listdir(dirpath)]
            if not runpaths:
                print "Found no runs in dictionary", dirpath
                sys.exit(1)
            
            for runpath in runpaths:
                endings = [name.split(".")[1] for name in os.listdir(runpath)]
                if "ini" not in endings:
                    print "No iniFile in runpath ", runpath
            ...
            print "Error with dictionary", sys.argv[1]
            sys.exit(1)
    except:
        print "Supplyrun path as cmlarg"
        sys.exit(1)
    ...
    ofile = open('/home/jorgmeister/scratch/texTable.tex', 'w')
