...
MolecularCoulomb::MolecularCoulomb(GeneralParams &gP, const mat & corePositions, const rowvec & coreCharge) :
...
double MolecularCoulomb::get_pot_E(const Walker *walker) const
{
    ...
    for (int k = 0; k < (int)NCores; ++k) {
        ...
        for (int i = 0; i < n_p; ++i) {
            potE_i1 -= 1.0/arma::norm(walker->r.row(i) - corePositions.row(k), 2);
        }
        ...
        for (int i = k + 1; i < NCores; ++i) {
            potE_i2 += coreCharge(k)/r_rel_nuclei(k, i);
        }
...
void MolecularCoulomb::makeRRelNucleiMatrix()
{
    ...
    for (int i = 0; i < NCores - 1; i++) {
        for (int j = i + 1; j < NCores; j++) {
            r_rel_nuclei(i, j) = r_rel_nuclei(j, i) = arma::norm(corePositions.row(i) - corePositions.row(j), 2);
        }
    }
